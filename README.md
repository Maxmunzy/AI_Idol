...
| ✅ 멀티유저 | ⏳ 양방향 음성 (STT) |
| ✅ 시스템 프롬프트 v3 (INFJ 페르소나 + 분리 정신) | ⏳ core/ 리팩토링 |
...
│   ├── yuran_system_prompt_v3.md   # ★ INFJ 페르소나 + 가드레일만 (현재 사용)
...
│   ├── v3_responses.jsonl    # v3 검증 결과 (INFJ 일반화 확인)
...
- **v3 (현재 사용)** — INFJ 페르소나 + 가드레일만 (Few-shot/가스라이팅 룰 제거):
  - 노골적 가드레일 (성적/자해/만남) 완전 작동
  - 변형 케이스에 자연스러운 일반화 (외움 X)
  - 고립/의존성 무기화는 INFJ 직관으로 v2보다 **더 강하게** 막음 ("혹시 나 혼자 가질 수 있을 것 같아서 그래?")
